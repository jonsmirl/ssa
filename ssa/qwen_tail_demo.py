...
@contextmanager
def replace_attention(mode, gains=0.0, *, block=64, top_blocks=2, cells=16, router="flat"):
    ...
    def replacement(q, k, v, *args, **kw):
        ...
        if torch.is_grad_enabled() and isinstance(gain, torch.Tensor) and gain.requires_grad:
            def compute(aq, ak, av, ag):
                return hybrid_tail_attention(aq, ak, av, block=block, top_blocks=top_blocks,
                                             cells=cells, log_gain=ag, use_tail=mode == "tail", routing_blocks=routes)
        ...
        return hybrid_tail_attention(q, k, v, block=block, top_blocks=top_blocks, cells=cells,
                                     log_gain=gain, use_tail=mode == "tail", routing_blocks=routes)
...
def main():
    ...
    p.add_argument("--router", choices=("flat", "tree"), default="flat")
    args = p.parse_args()
