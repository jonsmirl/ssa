...
transfer): the IVF router is 1.7× faster than the single-head flat GEMM at 4M and, at **8M (64 ms)**, the only
router that runs (the flat nb² matrix is 17 GB; the kernel's real router OOMs even earlier, ~1M) — the
sub-linear indexer the floor needs (the full-kernel landing remains a projection). So SubQ's claim is **achievable in principle under exactly
