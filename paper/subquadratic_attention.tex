...
Fixed-state tail correction & capped PPL gain; retrieval unresolved & capped full-test PPL improves at 512--32K; strict retrieval 2/9 plus one tie, sparse 4/9, dense 7/9\\
Prediction-preserving correction & numerical reference; expensive Qwen fallback & toy path checks; paired-endpoint Qwen policy retains 6/6 local probes with two forwards, no Qwen path certificate\\
...
\subsection{Full-scale frozen-tail test: length transfer fails}

The small-slice improvement does not survive longer contexts. A separate private ARC3-attached Kaggle run
on an RTX PRO 6000 Blackwell (94.97 GiB GPU, 176.88 GiB visible host RAM) evaluates the \emph{entire official
...
reset context boundaries. Previously inspected first test windows remain in the corpus; this enlarges
rather than replaces the holdout. Results depend on this specified tokenization/windowing protocol.
\begin{center}\small
\begin{tabular}{rrrrr}\toprule
Context & Dense PPL & Sparse PPL & Frozen-tail PPL & Tail-better windows\\\midrule
512 & 17.14 & 35.30 & 19.74 & 584/584\\
4096 & 12.10 & 74.09 & 57.84 & 62/73\\
8192 & 11.53 & 78.15 & 99.41 (worse) & 7/37\\
32768 & 11.05 & 183.01 & 261.45 (worse) & 1/10\\\bottomrule
\end{tabular}
\end{center}
...
dense/sparse/tail: 3/2/1 at 8K, 3/2/0 at 32K, and 1/0/0 at 128K, for totals \emph{7/9, 4/9, 1/9}. This is a
...
The full record is \texttt{runs/kaggle\_tail\_v1/ssa\_tail\_fullscale.json};
\texttt{kaggle\_tail/README.md} gives exact commands, offline dependencies, and notebook identity. Source/input
SHA256 manifest and remote source snapshots are retained, with base commit
\texttt{35c2c0b69338d67e9301da798ea1d04361b6d6a2}. All twelve manifest checks pass. The complete CUDA suite
reports 317 passed, 14 warnings. No theorem supplied favorable kernel-error bounds or learning/generalization
guarantees for these cells; this failure does not contradict the conditional math.

\subsection{Mean-consistent coarse reads and bounded influence}\label{sec:coarse-tail}

The fixed centers in (\ref{eq:tail-correction}) are assignment prototypes, not the means of the keys they
represent. Consequently $C_a^T\exp(\beta\inner{q}{c_a})$ need not lie below or above the true omitted
mass. Growing counts can amplify an inaccurate prototype score. A separate dense-oracle diagnostic uses
32 causal queries from the previously inspected Qwen layer-18/KV-head-0 8192-token fixture, 16 first-block
prototypes, and two exact 64-key blocks plus the current causal block. It uses a flat block-mean router,
not the full-scale batched tree, and is not an independent validation set.

\begin{table}[ht]
\centering\small
\begin{tabular}{@{}lrrrr@{}}\toprule
Tail estimator & Median mass ratio & Maximum & Overestimates & Mean output MSE\\\midrule
Prototypes, saved gain & 30.1722 & 102.7825 & 29/32 & 0.0855034\\
Omitted-cell means, zero gain & 0.054310 & 0.202241 & 0/32 & 0.0845644\\\bottomrule
\end{tabular}
\caption{Estimated/true omitted mass on one cached Qwen head; not a complete-model quality test.}
\end{table}
...
The record is \texttt{runs/tail\_mass\_diagnostic.json}, reproduced by
\texttt{python -m ssa.tail\_mass\_diagnostic --cache /tmp/ssa\_qwen\_qkv\_8192.npz --out
runs/tail\_mass\_diagnostic.json}. It records the fixture SHA256; the implementation base is
\texttt{35c2c0b69338d67e9301da798ea1d04361b6d6a2} plus the working-tree revision.
...
\begin{equation}\label{eq:coarse-jensen}
L_C=m_C e^{\beta\inner{q}{\bar k_C}+g_C}
\le m_C e^{\frac1{m_C}\sum_{i\in C}s_i}
\le\sum_{i\in C}e^{s_i}.
\end{equation}
This is a \emph{lower bound}, never an admissible omitted-mass upper cap. Empty cells contribute zero.
\texttt{tail\_correction.py} clips positive gains to zero in this mode; zero gains give the largest Jensen
lower bound for these summaries. Fixed causal assignments, count/key/value sums, and exact selected-key
...
\begin{equation}\label{eq:coarse-pythagoras}
...
leaves that optimum.
This does \emph{not} order output error, opposite-direction KL, retrieval accuracy, or downstream CE.
Appendix~\ref{app:coarse-tail} supplies the public derivation, the exact cost of gains, and the separate
upper-bound hypothesis needed to turn the partition gap into an output certificate.
...
$\alpha\|d\|^2\le2\inner{e}{d}$ for $\alpha>0$; a wrong-direction correction can fail at every
...
alignment results are machine-checked in Substrate commit
\texttt{206193290}. The self-contained public specification and streaming
pseudocode are copied into \nolinkurl{docs/coarse_read_projection.md}; no private repository is needed to
read or reproduce the mathematics. Appendix~\ref{app:coarse-tail} gives the public arguments and source
ownership. Numerical tests in \nolinkurl{ssa/tests/test_tail_correction.py} remain separate from those
proofs: CUDA arithmetic, future-independent implementation behavior, favorable learned cells, and CE or
retrieval quality are not formally verified.
The full-scale negative result above remains the measured status of the original frozen tail.

\subsection{Development validation: limiting influence helps more than changing the mean}
...
Candidate & 512 PPL & 8192 PPL & 32768 PPL\\\midrule
...
Prototypes, saved gains, share cap 0.25 & \textbf{12.14} & \textbf{47.36} & \textbf{145.03}\\
Actual means, zero gains, share cap 0.25 & 13.16 & 53.46 & 169.90\\\bottomrule
\end{tabular}
\caption{Development validation, two windows per length; bold identifies the selected revision, not every column's minimum.}
\end{table}
...
The complete record is \nolinkurl{runs/tail_revision_validation.json}, with per-window losses, source/input
hashes, and base commit \texttt{35c2c0b69338d67e9301da798ea1d04361b6d6a2} plus working-tree changes.
Reproduce with:
\begin{quote}\small\ttfamily\raggedright
python -m ssa.tail\_revision\_experiment \textbackslash{}\\
--split validation --contexts 512,8192,32768 \textbackslash{}\\
--windows 2 --out runs/tail\_revision\_validation.json
\end{quote}
Numerical hardening made after this process started replaces a subtracted log-mass offset with directly
capped mixture weights; the remote run uses that revised implementation, whose source hash
differs from the validation process's recorded hash. Algebraic equivalence is not a claim of
bitwise-identical corpus outputs or a floating-point proof.

\nolinkurl{runs/tail_revision_verification.json} records this command:
\begin{quote}\small\ttfamily\raggedright
OPENBLAS\_NUM\_THREADS=1 OMP\_NUM\_THREADS=1 \textbackslash{}\\
python -m pytest ssa/tests -q
\end{quote}
It reports \emph{349 passed, 14 warnings} in 56.08 s, with no skips. This suite overlaps the validation experiment,
so neither recorded runtime is an isolated performance measurement. This validation table alone
establishes no full-corpus or retrieval result; Section 5.14 reports the completed frozen rerun on the
already examined official test stream. The selection rule is recorded after examining validation, before
version-2 execution, in \nolinkurl{runs/tail_revision_selection.json}; neither that corpus nor the probes
are a pristine new holdout.

\subsection{Reader-weighted oracles: scalar versus joint-head suppression}
...
Output & Mean squared projected L2 error\\\midrule
Sparse & 33.66724\\
Prototype + cap & 24.55520\\
Raw-error oracle suppression & 24.50256\\
Reader-weighted oracle suppression & 24.48491\\
Joint 14-head feasible numerical suppression & 22.97759\\\bottomrule
\end{tabular}
\caption{One layer at fixed dense hidden inputs; the suppression oracle uses unavailable dense targets.}
\end{table}

The cap reduces this local projected error by 27.07\%, but even the reader-weighted oracle adds only
\emph{0.286\%} improvement over the cap and chooses the full capped correction for 88.5\% of queries.
...
shared oracle, find a feasible numerical solution with projected error 22.97759, \emph{6.42\% below the cap}.
...
Reproduce with:
\begin{quote}\small\ttfamily\raggedright
python -m ssa.reader\_weighted\_tail\_diagnostic \textbackslash{}\\
--context 8192 --layer 18 \textbackslash{}\\
--out runs/reader\_weighted\_tail\_diagnostic.json
\end{quote}
The artifact records tensor, projection, source, token, and gain hashes and the same base commit plus
working-tree changes. Attention outputs are bf16 and diagnostic projections float32 on the RTX 4080;
maximum numerical oracle excess over the better endpoint is $1.91\times10^{-6}$ for the scalar oracle. The three focused tests in
\nolinkurl{ssa/tests/test_reader_weighted_tail_diagnostic.py} pass. These are numerical checks, not a new
formally verified theorem or evidence of a significant new algorithm.

The connected route-aware formal object is now built in Substrate commit
\texttt{68968fc7}: route-switch overlap costs, stability of executed hard-routing comparisons, signed
nonlinear propagation, and terminal prediction margins. Section 5.15 and Appendix~\ref{app:routed-path}
state its conditional scope; \nolinkurl{docs/routed_correction_certificate.md} is the public standalone
specification. SSA now has a conditional float64 checker and a small random-transformer instantiation
(Section 5.16); \emph{a Qwen instantiation with useful uniform path bounds remains open}. The separate
...
\subsection{Full-scale capped tail: perplexity improves, retrieval is not preserved}
...
\centering\small
\begin{tabular}{lrrrr}\toprule
Context & Dense PPL & Sparse PPL & Original tail (v1) & Capped tail (v2)\\\midrule
512 & 17.14 & 35.30 & 19.74 & \textbf{23.43}\\
4096 & 12.10 & 74.09 & 57.84 & \textbf{50.80}\\
8192 & 11.53 & 78.15 & 99.41 & \textbf{72.88}\\
32768 & 11.05 & 183.01 & 261.45 & \textbf{154.77}\\\bottomrule
\end{tabular}
\caption{Frozen full-corpus regression comparison; the cap improves sparse PPL, not retrieval preservation.}
\end{table}
...
Retrieval does not follow CE. The capped arm has \emph{two strict wins and one tied maximum out of nine},
versus sparse 4/9 and dense 7/9. The script's ordered argmax records 3/9 for the capped arm because the
8K, depth-0.5 probe ties \texttt{walnut} and \texttt{lantern} at logit 4.15625 and the gold candidate comes
first. Strict capped wins by length are 2/3 at 8K, 0/3 at 32K, and 0/3 at 128K, with the one additional
tie at 8K. At 32K, sparse wins two probes and the capped correction wins none. The repeated fact and
four candidate words remain a narrow test; no broad retrieval or result for the motivating high-recall checkpoint
...
The record is \nolinkurl{runs/kaggle_tail_v2/ssa_tail_fullscale.json}, with remote source snapshots and
manifest under \nolinkurl{runs/kaggle_tail_v2/}. It records base commit
\texttt{35c2c0b69338d67e9301\allowbreak da798ea1d04361b6d6a2} plus hashed experiment sources, test tokens, gains,
and the frozen selection record. Build and submit the offline notebook with:
\begin{quote}\small\ttfamily\raggedright
python kaggle\_tail/build\_notebook.py --tail-mode prototype \textbackslash{}\\
--gain-mode saved --max-tail-share 0.25\\
kaggle kernels push -p kaggle\_tail -t 10800
\end{quote}
The private notebook is \nolinkurl{jonsmirl/ssa-tail-fullscale-rtx6000}, version 2, with ARC3 attached and
internet off; \nolinkurl{kaggle_tail/README.md} records attachment and dependency details. The final local
CUDA-enabled suite reports \emph{352 passed, 14 warnings in 36.49 s}, no skips, in
\nolinkurl{runs/tail_revision_verification.json}.
...
\subsection{Route-aware nonlinear acceptance certificate: mathematics built, Qwen bounds open}

Substrate commit \texttt{68968fc7aae0a0241722\allowbreak88f090f25c4d61451324} connects actual route changes
and nonlinear depth to a signed terminal read. Its 40 theorems do not assume the router globally Lipschitz.
For two nonempty selected sets at the \emph{same state}, with the same logits and values, write
$Z_A=\sum_{i\in A}e^{s_i}$. Their restricted distributions obey the exact overlap law
\begin{equation}\label{eq:routed-overlap}
\begin{split}
\operatorname{TV}(p_S,p_T)&=1-\frac{Z_{S\cap T}}{\max(Z_S,Z_T)},\\
\|o_S-o_T\|&\le2V\operatorname{TV}(p_S,p_T)\quad(\|v_i\|\le V).
\end{split}
\end{equation}
...
Appendix~\ref{app:routed-path} states the combined theorem: actual corrected executions, these route
alternatives, uniform derivative variation on shared state balls, and a closed radius recurrence imply a
signed terminal interval. Positive lower endpoints for every pairwise candidate margin preserve the
reference's strict winner on exactly that candidate set. The standalone public specification is
\nolinkurl{docs/routed_correction_certificate.md}. The proof owners are
{\small\texttt{RestrictedReadOverlap}, \texttt{ComparisonTrace}, \texttt{QuadraticRemainder},
\texttt{LinearPath}, \texttt{ComparisonPath}, \texttt{ComparisonPathWitness}}, and the attention
{\small\texttt{RoutedCorrectionRecognition}}.
...
\subsection{Float64 checker and a complete toy-transformer certificate}

\nolinkurl{ssa/routed_correction_certificate.py} implements a CPU float64 checker for supplied stage
evidence: reference and candidate states, full-state Jacobians, actual corrections, both branch evaluations
at the candidate state, uniform curvature/guard bounds, and compatible radii. It checks the recurrence,
observed Taylor residuals, shared tubes, same-state jumps, signed terminal intervals, and strict pairwise
margins. Missing or inconsistent evidence fails closed. A mandatory bound-provenance string is \emph{not
a proof}; the generic checker cannot establish the maps, derivatives, completeness of the trace, or
validity of uniform bounds from those data alone. Numerical slack is used, not outward-rounded
interval arithmetic.

\nolinkurl{ssa/routed_certificate_experiment.py} supplies a concrete small causal transformer: two layers,
two heads, eight tokens, hidden dimension four, top-two token reads, vocabulary seven, token/position
...
Algebraic margin passes before endpoint veto & 18/28\\
...
Maximum state-radius excess & $1.08\times10^{-16}$\\\bottomrule
\end{tabular}
\caption{Conditional float64 checker on a complete random toy transformer; not Qwen or an IEEE certificate.}
\end{table}
...
\begin{quote}\small\ttfamily\raggedright
python -m ssa.routed\_certificate\_experiment \textbackslash{}\\
--out runs/routed\_certificate\_reference.json
\end{quote}
The artifact stores all 28 paths, observed and algebraic checks, work counts, source hashes, and base
commit \texttt{35c2c0b69338d67e9301\allowbreak da798ea1d04361b6d6a2} plus working-tree changes. No
favorable Qwen curvature or executed-trace bounds have been obtained from this experiment.

\subsection{Qwen computed-endpoint acceptance: preservation with two charged forwards}

\nolinkurl{ssa/endpoint_acceptance.py} implements a separate, label-free fallback policy. Execute the
sparse reference and capped candidate independently. Accept the candidate's logits only when both have
the same \emph{unique} maximum on the supplied candidate set; otherwise return the computed reference
logits. Ties fail closed, nonfinite candidates are rejected, and nonfinite references are errors. This
preserves the computed reference's top-one choice by construction. It is \emph{not} an instantiation of
the Taylor/route-path certificate, a bound on exact-real Qwen logits, or evidence that its uniform
hypotheses hold. It evaluates both endpoints, rather than predicting their difference from a cheap certificate.
...
\centering\small
\begin{tabular}{lrrrr}\toprule
Context & Reference PPL & Candidate PPL & Policy PPL & Candidate accepted\\\midrule
512 & 16.71 & 12.08 & \textbf{14.91} & 75.83\%\\
8192 & 60.69 & 47.52 & \textbf{55.31} & 54.46\%\\
32768 & 180.91 & 144.96 & \textbf{166.23} & 45.28\%\\\bottomrule
\end{tabular}
\caption{Computed-endpoint acceptance on two development-validation windows per length, not a path certificate.}
\end{table}
...
This paired run gives \emph{reference 6/6, candidate 2/6, policy 6/6 strict gold wins}. At 32K, all three
...
Reproduce with:
\begin{quote}\small\ttfamily\raggedright
python -m ssa.routed\_acceptance\_demo --contexts 512,8192,32768 \textbackslash{}\\
--windows 2 --probe-contexts 8192,32768 \textbackslash{}\\
--out runs/routed\_acceptance\_qwen.json
\end{quote}
The complete artifact records both endpoint losses, acceptance/tie counts, charged work, peak allocations,
and source/input/gain hashes, with base commit
\texttt{35c2c0b69338d67e9301\allowbreak da798ea1d04361b6d6a2} plus working-tree changes. This establishes
...
The complete CUDA-enabled suite is run with:
\begin{quote}\small\ttfamily\raggedright
OPENBLAS\_NUM\_THREADS=1 OMP\_NUM\_THREADS=1 \textbackslash{}\\
python -m pytest ssa/tests -q
\end{quote}
It reports \emph{403 passed, 14 warnings in 40.13 s}. These tests check the implemented numerical behavior,
not the uniform-bound hypotheses for Qwen or a formally verified floating-point realization of the theorem.
...
\subsection{Variational coarse-tail projection and its limits}\label{app:coarse-tail}

The finite statements below are formalized in Substrate commit
\texttt{206193290} (full SHA in the public specification): \texttt{CellMeanProjection} owns the projection,
unique minimum, partition equality/refinement, gains, and supplied-upper-bound transport;
\texttt{SelectedCellMeanRead} owns selected-singleton refinement and the exact count/key/value realization.
\texttt{RestrictedReadOutputBound} owns the Pinsker/output transport, and \texttt{ResidualStep} owns
movement and the separate weak/strict alignment criteria. \texttt{CellMeanReadRecognition} applies these
results to attention. The public standalone specification is \nolinkurl{docs/coarse_read_projection.md}.
This proof status covers finite real mathematics, not the SSA kernels.
An independent Lean axiom audit of all 44 new theorems reports exactly
\texttt{[propext, Classical.choice, Quot.sound]} for every declaration.
...
within each cell satisfies (\ref{eq:coarse-pythagoras}), including competitors with zero cell masses.

\begin{proof}
Subtract the two KL sums to obtain
...
cell, $e^{a_C}$ times the value sum.
\end{proof}
...
\begin{equation}\label{eq:coarse-gain-kl}
\begin{split}
...
different objective. Given a \emph{separate} admissible total-mass upper bound $U\ge Z$, the zero-gain read
...
\begin{equation}\label{eq:coarse-alignment}
\|e-\alpha d\|^2-\|e\|^2
=\alpha^2\|d\|^2-2\alpha\inner{e}{d}.
\end{equation}
For positive $\alpha$, nonincrease is equivalent to the condition in Section~\ref{sec:coarse-tail}; strict
decrease uses a strict inequality. A formally supplied witness uses positive values $(2,1/2,7/2)$ and
dense weights $(6,5,1)$, selecting the first value. It gives $o_S=2$ and $o_{\rm dense}=3/2$. Positive
approximate tail weights $(1,5)$ on the same omitted values give $u=3$. For every positive cap $\rho$,
the allowed step $\alpha=\min(\rho,1)>0$ increases error from $1/2$ to $1/2+\alpha$. The norm movement
bound follows directly from homogeneity. These derivations assume exact finite summaries and real arithmetic. They supply neither
CUDA rounding guarantees nor favorable partitions, learning convergence, length transfer, or
subquadratic query work.

\subsection{Connected routed-path certificate and witnesses}\label{app:routed-path}

The following finite-stage theorem is formalized at Substrate \texttt{68968fc7}; the complete SHA and
public specification are given in Section 5.15. An independent Lean audit of all 40 new declarations
reports exactly \texttt{[propext, Classical.choice, Quot.sound]}. The path and remainder results hold in
real normed spaces, not only finite-dimensional Hilbert spaces. The overlap implementation uses a finite
coordinate carrier; arbitrary finite keys can be reindexed without changing the displayed sums.
...
\begin{equation}\label{eq:path-terms}
\begin{split}
e_j&=y_j-x_j,\qquad J_j=DF_{j,a_j}(x_j),\\
...
comparison satisfies the strict uniform-gap test from Section 5.15 on this \emph{same ball}, or supply
...
\begin{equation}\label{eq:path-radius}
\|J_j\|E_j+C_j+B_j+\tfrac12H_jE_j^2\le E_{j+1}.
\end{equation}

\paragraph{Conclusion.}
Every $\|e_j\|\le E_j$ and $\|r_j\|\le H_jE_j^2/2$. For a terminal continuous linear read $\ell$,
set $\lambda_m=\ell$ and $\lambda_j=\lambda_{j+1}\circ J_j$. Then
\begin{equation}\label{eq:path-interval}
\begin{split}
\ell(e_m)&=\lambda_0(e_0)+\sum_{j<m}\lambda_{j+1}(c_j+h_j+r_j),\\
D&=\lambda_0(e_0)+\sum_{j<m}\lambda_{j+1}(c_j+h_j),\\
R&=\sum_{j<m}\|\lambda_{j+1}\|\,H_jE_j^2/2,\qquad |\ell(e_m)-D|\le R.
\end{split}
\end{equation}
...
finite candidate set. Apply (\ref{eq:path-interval}) separately to each pairwise terminal logit read, with
nominal margin $m_{ab}$. If $m_{ab}+D_{ab}-R_{ab}>0$ for every competitor $b\ne a$, then $a$ remains
its strict winner. This is conditional reference preservation, not reference correctness or CE improvement.

\begin{proof}[Proof outline]
For the overlap law, the smaller probability on an intersection key is
$\frac{e^{s_i}}{\max(Z_S,Z_T)}$, while the minimum is zero elsewhere; normalized TV is one minus the sum of
...
alternative on the same balls, proving (\ref{eq:path-radius}) keeps each state within them. Applying
backward linear reads to the exact recurrence telescopes; bounding only omitted residual terms gives
(\ref{eq:path-interval}). Finally, adding each signed interval to its nominal pairwise margin proves
strict dominance.
\end{proof}

\paragraph{Nonlinear attained witness.}
Take $F_0(t)=t^2$, $F_1(t)=1-t$, with single-leaf routers, $x=(0,0,1)$, $y=(1/2,3/8,5/8)$, and
corrections $c=(1/8,0)$. Radii $(1/2,3/8,3/8)$ close (\ref{eq:path-radius}) with curvatures $(2,0)$
and zero branch jumps. For the identity terminal read, backward coefficients are $(0,-1,1)$, giving
$D=-1/8$, $R=1/4$, and actual displacement $-3/8$: the lower interval endpoint is attained. Candidate
reads $t$ and $0$ have nominal margin one and certified lower margin $5/8>0$. Thus the complete
certificate permits an actual nonlinear remainder and a nonzero injected correction.

\paragraph{Affine switch witness.}
Use $F_{\rm left}(t)=t$ at $t\ge0$, $F_{\rm right}(t)=t+1$ otherwise, and inputs $x_0=0$,
$y_0=-1/4$, with no injected correction. The branches differ and their difference at the common state
$y_0$ is $h_0=1$. Curvature is zero, so the actual terminal displacement $3/4$ equals $-1/4+1$ exactly.
The norm radius charges $1/4+1=5/4$, but the signed interval has zero width. This uses the actual-jump
alternative, not stability at a zero margin. More generally, identity-transported corrections $+1,-1$
have terminal effect zero although their norm sum is two.
