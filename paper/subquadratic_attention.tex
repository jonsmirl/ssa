...
its execution actually read, and perturb an unread one---the execution, hence the output, is unchanged. A
...
\bibitem{lean} J.~Smirl. \emph{Substrate Abstract Algebra} (Lean~4 development), namespace \texttt{Substrate.Inference.PhaseTransition}, sources under \texttt{Substrate/Inference/Algebra/} (\texttt{RetrievalMargin}, \texttt{SearchTradeoff}, \texttt{AnisotropicBound}, \texttt{SamuelsonBound}, \texttt{TemperedRouting}, \texttt{HierarchicalRouting}, \texttt{LosslessSelectionLimit}, \texttt{ValueAwareSelection}) and \texttt{Substrate/Inference/Shadow/}. Machine-checked, axiom-pure. In-depth technical details are available to qualified parties subject to approval.
\end{thebibliography}
