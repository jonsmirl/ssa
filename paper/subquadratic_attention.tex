...
left dense); the residual to $1.00$ is the frozen-key ceiling that only key co-adaptation (the routability
regularizer, \eqref{eq:reg}) closes. This is a routing-\emph{quality} measurement (the analytic,
score-materializing swap at moderate $n$), not the subquadratic-kernel regime; it validates the routing
theory and the co-adaptation thesis on a real frontier model.
...
\bibitem{lean} J.~Smirl. \emph{Substrate Abstract Algebra} (Lean~4 development), module \texttt{Substrate.Inference.PhaseTransition.Algebra} (\texttt{RetrievalMargin}, \texttt{SearchTradeoff}, \texttt{AnisotropicBound}, \texttt{SamuelsonBound}, \texttt{TemperedRouting}, \texttt{HierarchicalRouting}, \texttt{LosslessSelectionLimit}, \texttt{ValueAwareSelection}). Machine-checked, axiom-pure; available on request.
\end{thebibliography}
