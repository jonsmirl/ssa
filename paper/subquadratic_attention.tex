...
Samuelson's inequality. The limit is then proved: cheap, lossless, length-robust selection cannot hold
simultaneously for arbitrary keys (a one-line probe argument), so SSA's subquadratic \emph{exactness} is
...
$26.5\%$ to $4.2\%$ of keys at zero accuracy cost. The reduction was robust across head dimension and showed no
capacity trade-off down to $d=$ (number of clusters), since query-specific anisotropy needs only ${\sim}1$
...
multi-GPU or cluster budget). We leave that scaling, and the $12$M-token validation it would enable, to future
work with adequate compute.

\paragraph{What the headline retrieval numbers do and do not show.} Reported single-target
needle-in-a-haystack accuracies of $98$--$100\%$ at $10^6$--$10^7$ tokens are real and consistent
with~\eqref{eq:recovery}--\eqref{eq:detect}, but they live in a specific regime. Retrieval was measured as a
