...
selected block indices \emph{directly} removes both.
...
\hline
256K & $0.21$\,ms & $7.1$\,ms & flat $33.8\times$\\
2M & $12.9$\,ms & $18.9$\,ms & flat $1.5\times$\\
4M & OOM ($nb^2{=}4.3$\,GB) & $31.4$\,ms & IVF (flat dead)\\
8M & OOM ($17.2$\,GB) & $61.7$\,ms & IVF (flat dead)\\
\hline
\end{tabular}
\end{center}

\noindent The crossover is ${\sim}3\mathrm{M}$, and the IVF is the only router that runs past the flat
router's $4\mathrm{M}$ memory wall. At $12\mathrm{M}$ the IVF router (${\sim}90$\,ms, extrapolated) is small
