...
\midrule
$0.02$ & $89.0$ & $187.6$ & $720.3$ & $8.1\times$\\
$0.05$ & $81.0$ & $453.6$ & $975.4$ & $12.0\times$\\
$0.10$ & $74.6$ & $1340.2$ & $2165.0$ & $29.0\times$\\
$0.20$ & $70.2$ & $3343.4$ & $3685.8$ & $52.5\times$\\
$0.40$ & $64.1$ & $3946.1$ & $4045.3$ & $63.1\times$\\
\bottomrule
\end{tabular}
\end{center}
...
Proposition~\ref{prop:tight} says why it cannot. Second, \emph{reading keys is worth roughly $4\times$ at
benign geometry} ($187.6$ against $720.3$ at spread $0.02$), which is the first absolute justification for the
anisotropic refinement~\eqref{eq:aniso}; $\rho_c$ is not a minor sharpening but most of the distance to the
floor. Third, the partition price is nearly flat in the spread ($89.0\to 64.1$) while the summary price moves
...
\paragraph{Relation to the formalized ceiling.} Proposition~\ref{prop:tight} instantiates an abstract result
...
zero violations on clustered and random geometry). The component table is the trilemma made concrete:
...
\bibitem{lean} J.~Smirl. \emph{Substrate Abstract Algebra} (Lean~4 development), namespace \texttt{Substrate.Inference.PhaseTransition}, sources under \texttt{Substrate/Inference/Algebra/} (\texttt{RetrievalMargin}, \texttt{SearchTradeoff}, \texttt{AnisotropicBound}, \texttt{SamuelsonBound}, \texttt{TemperedRouting}, \texttt{HierarchicalRouting}, \texttt{LosslessSelectionLimit}, \texttt{LinearReadCeiling}, \texttt{ValueAwareSelection}) and \texttt{Substrate/Inference/Shadow/}. Machine-checked, axiom-pure. In-depth technical details are available to qualified parties subject to approval.
\end{thebibliography}
