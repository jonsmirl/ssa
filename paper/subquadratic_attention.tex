...
``recall'' every spike without probing one. Likewise, this theorem does not cover an arbitrary preprocessed
index whose stored side information can name unprobed positions; applying a lower bound to such an index
requires a model and budget for that information. For grounded adaptive probes, however, worst-case
...
the audit through private Substrate commit \texttt{130cae3e9} machine-checks the corresponding results for
