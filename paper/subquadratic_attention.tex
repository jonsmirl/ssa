...
long-context language models. We present \emph{Subquadratic Sparse Attention} (SSA), an attention mechanism
...
layer runs in $O(n\sqrt{n})$ flat or near-linear with a hierarchical router. We develop the theory that says
...
distractors; selection works because it cuts $\mu$ from $n$ to $\kappa$, making recovery \emph{flat in $n$}. We
give the admissible routing bound that licenses summary-only selection, a tempered (cumulant) routing score
that---unlike centroid routing---sees in-block outliers, and a closed-form variance prune test from
Samuelson's inequality. We then prove the limit: cheap, lossless, length-robust selection cannot hold
...
manufacture, via a routability regularizer that shrinks off-target spread. Finally we show length
...
of a dense model given equal training while attending $38\%$ of keys). All claims are accompanied by
...
objects---upper bounds that make selection provably lossless---and the prune test SSA uses.
...
would weight non-negligibly, and \emph{length-robust} if its accuracy is flat in $n$. The bounds above let us
state the fundamental limit.
...
cannot: its embeddings past the trained length are untrained. This predicts, and we observe, zero-shot
...
with~\eqref{eq:recovery}--\eqref{eq:detect}, but they live in a specific regime. We measured retrieval as a
