...
def _route_head(mu, qb, top_c, local, nprobe, search_k, res, query_blocks=None):
    ...
    qi = (torch.arange(nb, device=mu.device) if query_blocks is None
          else query_blocks.to(device=mu.device))
    if qi.shape != (qb.shape[0],):
        raise ValueError("query_blocks must provide one causal block index per query summary")
...
def ivf_route_gqa(q, k, block=BLOCK, top_c=8, local=1, nprobe=4, search_k=None, res=None):
    """GQA-aware IVF routing without materializing repeated key/value heads.

    ``q`` is ``(B,Hq,n,d)`` and ``k`` is ``(B,Hkv,n,d)``.  One IVF index is built per KV head and
    searched by every query head in its GQA group.  This is selection-equivalent to repeating K and
    calling :func:`ivf_route`, but avoids rebuilding the same index ``Hq/Hkv`` times.
    """
    B, Hq, n, d = q.shape
    ...
    W = min(top_c + local + 1, nb)
    kv_num = torch.empty(B, Hq, nb, device=q.device, dtype=torch.int32)
    kv_idx = torch.empty(B, Hq, nb, W, device=q.device, dtype=torch.int32)
    q_blocks = torch.arange(nb, device=q.device).repeat(groups)
    for batch in range(B):
        for hk in range(Hkv):
            mu = block_means(k[batch, hk], block)
            first = hk * groups
            qb = torch.cat([block_means(q[batch, h], block)
                            for h in range(first, first + groups)], dim=0)
            kn, ki = _route_head(mu, qb, top_c, local, nprobe, search_k, res,
                                 query_blocks=q_blocks)
            kv_num[batch, first:first + groups] = kn.view(groups, nb)
            kv_idx[batch, first:first + groups] = ki.view(groups, nb, W)
            del mu, qb
    return kv_num, kv_idx
