...
def run():
    ...
    RESULT["router"] = {
        **production_cfg.__dict__,
        "complexity": "fixed selected blocks/token; online center-radius tree; fixed outlier reservoir",
        "strict_causal": True, "native_gqa": True,
        "geometry_note": "pre-RoPE Q/K select content candidates; post-RoPE Q/K compute attention",
    }
    dump()
    ...
    main_ids, main_labels, main_gold, main_metadata = make_choice_niah_ids(
        tokenizer, N_MAIN, depth=0.5, device="cuda", return_metadata=True)
    RESULT["main_prompt"] = main_metadata
    ...
    main_logits, main_stats = streamer(
        main_ids, progress=progress, probe_block=main_metadata["needle_block"])
    RESULT["main"] = {
